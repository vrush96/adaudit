...
Function Get-DomainControllerDetails {
    ...
    try {
        # Get all Domain Controllers
        $domainControllers = Get-ADDomainController -Filter * | Select-Object Name, IPv4Address, Site,OperatingSystem |Export-Csv -Path $OutputFile -NoTypeInformation

        # Initialize an array to hold the details
        $dcDetails = @()

        foreach ($dc in $domainControllers) {
            # Ensure both Name and IPAddress properties are available
            if ($dc.Name -and $dc.IPv4Address) {
                $dcDetails += "$($dc.Name) - $($dc.IPv4Address) - $($dc.Site) - $($dc.OperatingSystem)"
            }
        }

        # Define the output file path
        $outputFilePath = Join-Path -Path $outputdir -ChildPath "DC_Details.txt"

        # Write the details to the output file
        $dcDetails | Out-File -FilePath $outputFilePath

        Write-Host "Details of all Domain Controllers (name and IP address) have been saved to $outputFilePath"
    }
    catch {
        Write-Host "An error occurred: $_"
    }
