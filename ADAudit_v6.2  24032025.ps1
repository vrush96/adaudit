<#
    ...
    .CHANGELOG
    V6.0, 25/05/2024 - Initial version.
    V6.1, 11/06/2024 - Added GPO name & count,AD Group name and count, ReplicationSummary between DC's, DC name with IPAddress & DC Firewall Profiles.
    v6.2, 17/06/2024 - Added InactiveComputers, Checking both Microsoft or Windows Laps Configuration, Added EmptyOus, Output Folder Handled(<Domain Name> Ad Audit Report today's date), Added LastLogon for admins in Get-PrivilegedGroupAccounts function with ServerName LastLogon retrieved from, Updated Get-DomainControllerDetails function for Dc details files and added site and OperatingSystem info.
...
Function Write-Both() {
    ...
    Write-Host "$args"
    Add-Content -Path "$outputdir\adauditlogs.txt" -Value "$args"
}
...
function Get-LAPSStatus {
    ...
    if (-not (Get-Module -ListAvailable -Name ActiveDirectory)) {
        Write-Host "The Active Directory module is not available. Please install it and try again."
        return
    }

    # Check for presence of Microsoft LAPS in domain
    try {
        $schemaObject = Get-ADObject "CN=ms-Mcs-AdmPwd,CN=Schema,CN=Configuration,$((Get-ADDomain).DistinguishedName)" -ErrorAction Stop
        Write-Both "    [+] Microsoft LAPS is installed in domain" -ForegroundColor Green
    }
    catch {
        Write-Both "    [!] Microsoft LAPS is not installed in domain (KB258)" -ForegroundColor Red
        Write-Nessus-Finding "LAPSMissing" "KB258" "Microsoft LAPS is not installed in domain"
    }

    # Check for Windows LAPS attributes
    try {
        $windowsLapsObject = Get-ADComputer -Filter * -Properties ms-Mcs-AdmPwd, ms-Mcs-AdmPwdExpirationTime
        Write-Both "    [+] Windows LAPS attributes found in domain" -ForegroundColor Green
    }
    catch {
        Write-Both "    [!] Windows LAPS attributes not found in domain" -ForegroundColor Red
        Write-Nessus-Finding "LAPSMissing" "KB258" "Windows LAPS attributes not found in domain"
    }
    ...
    if (Get-Module -ListAvailable -Name AdmPwd.PS) {
        ...
        # Check for computers without LAPS password
        $missingComputers = Get-ADComputer -Filter { ms-Mcs-AdmPwd -notlike "*" } | Select-Object -ExpandProperty Name
        $missingCount = $missingComputers.Count
        if ($missingCount -gt 0) {
            $missingComputers | Out-File -FilePath "$outputdir\laps_missing-computers.txt"
            Write-Host "    [!] Some computers/servers don't have LAPS password set. See $outputdir\laps_missing-computers.txt" -ForegroundColor Yellow
            Write-Nessus-Finding "LAPSMissingorExpired" "KB258" ([System.IO.File]::ReadAllText("$outputdir\laps_missing-computers.txt"))
        }

        # Check for expired passwords
        $computersList = Get-ADComputer -Filter { ms-Mcs-AdmPwdExpirationTime -like "*" } -Properties ms-Mcs-AdmPwdExpirationTime | Select-Object Name, ms-Mcs-AdmPwdExpirationTime
        $expiredCount = 0
        foreach ($computer in $computersList) {
            ...
            if ($expiration -lt $today) {
                $expiredCount++
                "$($computer.Name) password expired since $expiration" | Out-File -FilePath "$outputdir\laps_expired-passwords.txt" -Append
            }
        }
        if ($expiredCount -gt 0) {
            Write-Host "    [!] Some computers/servers have LAPS password expired. See $outputdir\laps_expired-passwords.txt" -ForegroundColor Yellow
            Write-Nessus-Finding "LAPSMissingorExpired" "KB258" ([System.IO.File]::ReadAllText("$outputdir\laps_expired-passwords.txt"))
        }

        # Check for LAPS extended rights
        Get-ADOrganizationalUnit -Filter * | Find-AdmPwdExtendedRights -PipelineVariable OU | ForEach-Object {
            $_.ExtendedRightHolders | ForEach-Object {
                if ($_ -ne $null) {
                    "$_ can read password attribute of $($OU.ObjectDN)" | Out-File -FilePath "$outputdir\laps_read-extendedrights.txt" -Append
                }
            }
        }
        Write-Host "    [!] LAPS extended rights exported. See $outputdir\laps_read-extendedrights.txt" -ForegroundColor Yellow
        Write-Nessus-Finding "LAPSMissingorExpired" "KB258" ([System.IO.File]::ReadAllText("$outputdir\laps_read-extendedrights.txt"))
    }
    else {
        Write-Host "    [!] LAPS PowerShell module is not installed. Cannot run LAPS checks on this DC" -ForegroundColor Red
    }
}
Function Get-PrivilegedGroupAccounts {
    # Lists users in Administrators, Domain Admins, and Enterprise Admins groups along with their last logon date
    ...
    $privUsersUnique = $privilegedUsers | Sort-Object -Property SamAccountName -Unique
    $count = 0
    $totalCount = ($privUsersUnique | Measure-Object).Count
    ...
    foreach ($account in $privUsersUnique) {
        ...
        # Get the last logon date for the user and convert it to a readable date
        $user = Get-ADUser -Identity $account.SamAccountName -Properties LastLogon
        $lastLogon = if ($user.LastLogon -ne 0) { [DateTime]::FromFileTime($user.LastLogon).ToString("yyyy-MM-dd HH:mm:ss") } else { "Never logged on" }
        $outputLine = "$($account.SamAccountName) ($($account.Name)) - Last Logon: $lastLogon - LastLogon retrieved from $env:ComputerName"

        Add-Content -Path "$OutputDir\accounts_userPrivileged.txt" -Value $outputLine
        $count++
    }
    ...
    if ($count -gt 0) {
        Write-Both "    [!] There are $count accounts in privileged groups LastLogon retrieved from $env:ComputerName, see accounts_userPrivileged.txt"
    }
}
function Get-ReplicationSummary {
    ...
    $outputFile = "$outputdir\replication_summary.txt"
    
    # Run the repadmin /replsum command and capture the output
    $repadminOutput = & repadmin /replsum 2>&1

    # Write the output to a file
    $repadminOutput | Out-File -FilePath $outputFile

    # Process the output (example: counting lines)
    $totalcount = $repadminOutput.Count
    $count = 0

    foreach ($line in $repadminOutput) {
        $count++
        Write-Progress -Activity "Processing replication summary..." -Status "Currently processed $count of $totalcount lines" -PercentComplete ($count / $totalcount * 100)
    }
    
    Write-Progress -Activity "Processing replication summary..." -Status "Ready" -Completed
    
    if ($totalcount -gt 0) {
        Write-Both "    [!] Replication summary completed, see replication_summary.txt"
        Write-Nessus-Finding "ReplicationSummary" "KBXXX" ([System.IO.File]::ReadAllText($outputFile))
    }
}
...
Function Get-AllGPOs {
    ...
    # Retrieve all GPOs in the AD
    $gpos = Get-GPO -All
    $gpoCount = $gpos.Count
    
    # Define the output file
    $outputFile = "$outputdir\all_GPOs.txt"
    
    # Initialize progress variables
    $count = 0
    $totalcount = $gpoCount
    
    # Process each GPO and write to the output file
    foreach ($gpo in $gpos) {
        if ($totalcount -eq 0) { break }
        Write-Progress -Activity "Retrieving all GPOs..." -Status "Currently processed $count out of $totalcount GPOs" -PercentComplete ($count / $totalcount * 100)
        Add-Content -Path $outputFile -Value "$($gpo.DisplayName) ($($gpo.Id))"
        $count++
    }
    
    Write-Progress -Activity "Retrieving all GPOs..." -Status "Ready" -Completed
    
    # Write the count to the output file
    Add-Content -Path $outputFile -Value "Total GPOs: $gpoCount"

    if ($gpoCount -gt 0) {
        Write-Both "    [!] There are $gpoCount GPOs in Active Directory, see all_GPOs.txt"
        Write-Nessus-Finding "GPOs" "KBXXX" ([System.IO.File]::ReadAllText($outputFile))
    }
}
Function Get-AllADGroups {
    ...
    # Retrieve all groups in the AD
    $groups = Get-ADGroup -Filter *
    $groupCount = $groups.Count
    
    # Define the output file
    $outputFile = "$outputdir\all_AD_groups.txt"
    
    # Initialize progress variables
    $count = 0
    $totalcount = $groupCount
    
    # Process each group and write to the output file
    foreach ($group in $groups) {
        if ($totalcount -eq 0) { break }
        Write-Progress -Activity "Retrieving all AD groups..." -Status "Currently processed $count out of $totalcount groups" -PercentComplete ($count / $totalcount * 100)
        Add-Content -Path $outputFile -Value "$($group.SamAccountName) ($($group.Name))"
        $count++
    }
    
    Write-Progress -Activity "Retrieving all AD groups..." -Status "Ready" -Completed
    
    # Write the count to the output file
    Add-Content -Path $outputFile -Value "Total AD Groups: $groupCount"

    if ($groupCount -gt 0) {
        Write-Both "    [!] There are $groupCount groups in Active Directory, see all_AD_groups.txt"
        Write-Nessus-Finding "ADGroups" "KBXXX" ([System.IO.File]::ReadAllText($outputFile))
    }
}
Function Get-DCFirewallProfiles {
    ...
    # Ensure the NetSecurity module is available
    if (-not (Get-Module -ListAvailable -Name NetSecurity)) {
        Write-Host "The NetSecurity module is not available. Please install it and try again."
        return
    }
    ...
    try {
        ...
        # Initialize an array to hold the firewall profile details
        $firewallDetails = @()

        foreach ($dc in $domainControllers) {
            # Skip DCs with no Name or DNSHostName
            if (-not $dc.Name -or -not $dc.HostName) {
                continue
            }

            try {
                # Use Invoke-Command to run Get-NetFirewallProfile on each DC
                $profiles = Invoke-Command -ComputerName $dc.HostName -ScriptBlock {
                    Get-NetFirewallProfile | Select-Object Name, @{Name='Enabled';Expression={if ($_.Enabled) { 'True' } else { 'False' }}}
                }

                # Append the results to the details array
                foreach ($profile in $profiles) {
                    $firewallDetails += "$($dc.Name) - $($profile.Name): $($profile.Enabled)"
                }
            }
            catch {
                Write-Host "Could not retrieve firewall profile for DC: $($dc.Name)"
            }
        }

        # Define the output file path
        #$outputFilePath = Join-Path -Path $outputdir -ChildPath "DC_Firewall_Profiles.txt"

        # Write the details to the output file
        $firewallDetails | Out-File -FilePath $outputdir\DC_Firewall_Profiles.txt

        Write-Host "Firewall profile details for all Domain Controllers have been saved to $outputFilePath"
    }
    catch {
...
Function Get-InactiveComputers {
    ...
    $count = 0
    $progresscount = 0
    $inactiveComputers = Search-ADAccount -AccountInactive -Timespan (New-TimeSpan -Days 180) -ComputersOnly | Where-Object { $_.Enabled -eq $true }
    $totalcount = ($inactiveComputers | Measure-Object | Select-Object -ExpandProperty Count)

    foreach ($computer in $inactiveComputers) {
        if ($totalcount -eq 0) { break }
        $progresscount++
        Write-Progress -Activity "Searching for inactive computers..." -Status "Currently identified $count" -PercentComplete ($progresscount / $totalcount * 100)

        if ($computer.Enabled) {
            if ($computer.LastLogonDate) {
                $computerLastUsed = $computer.LastLogonDate
            }
            else {
                $computerLastUsed = "Never"
            }
            Add-Content -Path "$outputdir\computers_inactive.txt" -Value "Computer $($computer.SamAccountName) ($($computer.Name)) has not logged on since $computerLastUsed"
            $count++
        }
    }

    Write-Progress -Activity "Searching for inactive computers..." -Status "Ready" -Completed

    if ($count -gt 0) {
        Write-Both "    [!] $count inactive computer accounts (180 days), see computers_inactive.txt"
        Write-Nessus-Finding "InactiveComputers" "KB500" ([System.IO.File]::ReadAllText("$outputdir\computers_inactive.txt"))
    }
}
Function Get-EmptyOUs {

    # Initialize the hashtable for empty OUs
    $emptyOUs = @{}

    # Get all OUs
    $ous = Get-ADOrganizationalUnit -Filter *
    $totalcount = $ous.Count
    $count = 0

    foreach ($ou in $ous) {
        Write-Progress -Activity "Checking OUs for objects..." -Status "Currently checked $count OUs" -PercentComplete (($count / $totalcount) * 100)

        # Check if the OU contains any objects other than OUs
        $containsObjects = Get-ADObject -Filter { ObjectClass -ne 'organizationalUnit' } -SearchBase $ou.DistinguishedName
        
        if ($containsObjects.Count -eq 0) {
            # If the OU is empty, add it to the hashtable
            if (-not $emptyOUs.ContainsKey($ou.DistinguishedName)) {
                $emptyOUs[$ou.DistinguishedName] = $ou.Name
            }
        }
    ...
    Write-Progress -Activity "Checking OUs for objects..." -Status "Ready" -Completed

    # Save the empty OUs to a text file
    $emptyOUsFile = Join-Path -Path $outputDir -ChildPath "empty_ous.txt"

    # Add header
    "OU Name, OU Path" | Out-File -FilePath $emptyOUsFile -Encoding utf8

    # Add the data
    $emptyOUs.GetEnumerator() | Sort-Object Key | ForEach-Object { 
        "$($_.Value), $($_.Key)" | Out-File -FilePath $emptyOUsFile -Append -Encoding utf8
    }
    
    Write-Both "    [!] Empty OUs saved to $emptyOUsFile"
}
...
function Get-DomainControllerDetails {
    param (
        [string]$OutputDir = "C:\$env:UserDomain Ad Audit Report $starttime"
    )
    ...
    try {
        ...
        $csvOutputFile = Join-Path -Path $OutputDir -ChildPath "Alldcs.csv"
        $txtOutputFile = Join-Path -Path $OutputDir -ChildPath "DC_Details.txt"

        # Get all Domain Controllers
        $domainControllers = Get-ADDomainController -Filter * | Select-Object Name, IPv4Address, Site, OperatingSystem
        
        # Export to CSV
        $domainControllers | Export-Csv -Path $csvOutputFile -NoTypeInformation

        # Prepare details for text file
        $dcDetails = $domainControllers | ForEach-Object {
            "$($_.Name) - $($_.IPv4Address) - $($_.Site) - $($_.OperatingSystem)"
        }

        # Write the details to the output text file
        $dcDetails | Out-File -FilePath $txtOutputFile

        Write-Host "Details of all Domain Controllers have been saved to:"
        Write-Host "CSV File: $csvOutputFile"
        Write-Host "Text File: $txtOutputFile"
    }
    catch {
...
$starttime = Get-Date -Format "ddMMyyyy"
$outputdir = "C:\$env:UserDomain Ad Audit Report $starttime" #(Get-Item -Path "C:\Agreem Test Script").FullName + "\" + $env:computername
...
if ($domainaudit -or ($all -and 'domainaudit' -notin $exclude) -or 'domainaudit' -in $selectedChecks) { $running = $true ; Write-Both "[*] Domain Audit" ; Get-LastWUDate ; Get-DCEval ; Get-TimeSource ; Get-PrivilegedGroupMembership ; Get-MachineAccountQuota; Get-DefaultDomainControllersPolicy ; Get-SMB1Support ; Get-FunctionalLevel ; Get-DCsNotOwnedByDA ; Get-ReplicationType ; Check-Shares ; Get-RecycleBinState ; Get-CriticalServicesStatus ; Get-RODC; Get-ReplicationSummary; Get-AllGPOs; Get-AllADGroups; Get-DomainControllerDetails -OutputDir $outputdir; Get-DCFirewallProfiles; Get-InactiveComputers; Get-EmptyOUs }
